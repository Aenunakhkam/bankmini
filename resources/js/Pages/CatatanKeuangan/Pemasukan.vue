...
const onSearch = () => {
    searchForm.get(route('catatan-keuangan.pemasukan'), { preserveState: true });
};
...
<template>
    <Head title="Catatan Pemasukan" />

    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center gap-3">
                <div class="p-2 bg-green-100 rounded-xl shadow-sm">
                ...
                <div>
                    <h2 class="font-bold text-xl text-gray-800 leading-tight">Daftar Pemasukan (Setoran)</h2>
                    <p class="text-sm text-gray-500 font-medium">Riwayat uang masuk dari nasabah Bank Mini</p>
                </div>
            </div>
        </template>
                    ...
                    <div class="p-6 border-b border-gray-100 flex flex-col md:flex-row justify-between items-center gap-4">
                        <div class="relative w-full md:w-96">
                            ...
                            <TextInput 
                                ...
                                type="text" 
                                class="w-full pl-10 pr-4 py-2 rounded-xl border-gray-200 focus:ring-green-600 focus:border-green-600 bg-gray-50 focus:bg-white" 
                                placeholder="Cari nama atau rekening nasabah..." 
                            />
                        </div>
                        <div class="flex items-center gap-2 mt-4 md:mt-0">
                            <a :href="route('catatan-keuangan.pemasukan.pdf')" target="_blank" class="px-4 py-2 rounded-xl text-sm font-bold bg-red-500 hover:bg-red-600 text-white shadow transition-all flex items-center gap-2">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                                PDF (Landscape)
                            </a>
                            <a :href="route('catatan-keuangan.pemasukan.excel')" target="_blank" class="px-4 py-2 rounded-xl text-sm font-bold bg-green-500 hover:bg-green-600 text-white shadow transition-all flex items-center gap-2">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                                Excel
                            </a>
                        </div>
                        ...
                        <table class="w-full text-sm text-left text-gray-500">
                            <thead class="text-xs text-gray-500 uppercase tracking-wider bg-gray-50 border-b border-gray-100">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-4 text-right font-bold text-gray-600">Nominal Setoran</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr v-for="tx in transactions.data" :key="tx.id" class="bg-white border-b last:border-0 hover:bg-green-50/50 transition-colors">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right font-bold text-green-600 text-base">
                        ...
                        <div class="flex space-x-1">
                            <template v-for="(link, p) in transactions.links" :key="p">
                                <a 
                                    ...
                                    :class="link.active ? 'bg-green-600 text-white border-green-600' : 'bg-white text-gray-700 hover:bg-gray-50 border-gray-300'"
                                    v-html="link.label"
                                ></a>
